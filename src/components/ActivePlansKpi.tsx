...
export function ActivePlansKpi({
  activeActions,
  today,
  isAdmin,
  onGenerateReport,
  ...
  const activeByExec = useMemo(() => {
    ...
    for (const action of activeActions) {
      ...
      map.get(exec)!.push({ name: action.client_name, id: action.id })
    }
    return map
  }, [activeActions, today])
  ...
  const executiveCounts = useMemo(
    () =>
      Array.from(activeByExec.entries())
  ...
  const filteredCounts =
    selectedExec === 'all'
      ? executiveCounts
      : executiveCounts.filter((e) => e.name === selectedExec)
  ...
  const handleGerarRelatorio = () => {
    ...
    const selected = activeActions.find((a) => a.id === selectedActionId)
    if (!selected) {
      setShowSelectHint(true)
      toast.info('Selecione um plano na lista para gerar o relatório.')
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {showSelectHint && (
              <span className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1 animate-fade-in">
                <AlertCircle className="h-3.5 w-3.5" /> Selecione um plano
              </span>
            )}
            {onGenerateReport && (
              <Button
        ...
        {filteredCounts.length > 0 ? (
          <div className="max-h-[320px] overflow-y-auto space-y-1 pr-1">
            {filteredCounts.map((item, idx) => {
              ...
              const agencies = activeByExec.get(item.name) ?? []
              return (
                    ...
                    <div className="ml-10 mb-2 space-y-1 animate-fade-in">
                      {agencies.length > 0 ? (
                        agencies.map((agency, i) => (
                          <div
                            key={`${agency.id}-${i}`}
                            onClick={() => setSelectedActionId(agency.id)}
                            className={cn(
                              'flex items-center gap-2 py-1 px-2 rounded text-xs hover:bg-muted/30 transition-colors cursor-pointer',
                              selectedActionId === agency.id
                          ...
                          >
                            ...
                            {selectedActionId === agency.id && (
                              <FileText className="h-3 w-3 text-primary ml-auto shrink-0" />
                            )}
                          </div>
                        ))
                      ) : (
