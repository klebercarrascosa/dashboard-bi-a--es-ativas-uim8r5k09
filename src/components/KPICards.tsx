...
export function KPICards({ data, activeTab }: KPICardsProps) {
  const totalVenda = data.reduce((acc, row) => acc + row.venda, 0)
  const totalVendaLY = data.reduce((acc, row) => acc + row.vendaLY, 0)
  const deltaTotal = totalVenda - totalVendaLY
  const pctYoYTotal = totalVendaLY > 0 ? ((totalVenda - totalVendaLY) / totalVendaLY) * 100 : 0
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      {/* Delta LY & % YoY */}
      <Card
        className={`shadow-sm ${isPositive ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-rose-500/30 bg-rose-500/5'}`}
      >
        <CardContent className="p-5">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
            ...
            <div
              className={`rounded-lg p-2 ${isPositive ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 text-rose-600 dark:text-rose-400'}`}
            >
              {isPositive ? (
          ...
          <div className="mt-2">
            <div className="flex items-baseline justify-between">
              <h3
                className={`text-2xl font-extrabold tracking-tight ${isPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
              >
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {isPositive ? 'Crescimento acima de LY' : 'Queda em relação a LY'}
            </p>
          </div>
        </CardContent>
      </Card>
